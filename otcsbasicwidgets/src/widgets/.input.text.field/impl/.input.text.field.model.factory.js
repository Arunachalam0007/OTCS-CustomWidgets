define([
  ...
  'otcsb/widgets/.input.text.field/impl/.input.text.field.model'     // Model to create the factory for
], function (ModelFactory, ConnectorFactory, InputTextFieldModel) {
  'use strict';

  var InputTextFieldModelFactory = ModelFactory.extend({
    // Unique prefix of the default model instance, when this model is placed
    // to a context to be shared by multiple widgets
    propertyPrefix: '.input.text.field',

    constructor: function InputTextFieldModelFactory(context, options) {
      ...
      this.property = new InputTextFieldModel(undefined, {
        connector: connector
      });
    },
  ...
  return InputTextFieldModelFactory;
});
