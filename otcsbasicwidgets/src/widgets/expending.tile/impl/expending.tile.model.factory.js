define([
  ...
  'otcsb/widgets/expending.tile/impl/expending.tile.model'     // Model to create the factory for
], function (ModelFactory, ConnectorFactory, ExpendingTileModel) {
  'use strict';

  var ExpendingTileModelFactory = ModelFactory.extend({
    // Unique prefix of the default model instance, when this model is placed
    // to a context to be shared by multiple widgets
    propertyPrefix: 'expending.tile',

    constructor: function ExpendingTileModelFactory(context, options) {
      ...
      this.property = new ExpendingTileModel(undefined, {
        connector: connector
      });
    },
  ...
  return ExpendingTileModelFactory;
});
