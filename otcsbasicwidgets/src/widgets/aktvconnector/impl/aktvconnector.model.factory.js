define([
  ...
  'otcsb/widgets/aktvconnector/impl/aktvconnector.model'     // Model to create the factory for
], function (ModelFactory, ConnectorFactory, AktvconnectorModel) {
  'use strict';

  var AktvconnectorModelFactory = ModelFactory.extend({
    // Unique prefix of the default model instance, when this model is placed
    // to a context to be shared by multiple widgets
    propertyPrefix: 'aktvconnector',

    constructor: function AktvconnectorModelFactory(context, options) {
      ...
      this.property = new AktvconnectorModel(undefined, {
        connector: connector
      });
    },
  ...
  return AktvconnectorModelFactory;
});
